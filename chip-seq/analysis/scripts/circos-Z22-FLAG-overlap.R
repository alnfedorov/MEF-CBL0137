...
for (i in 1:length(colors)) {
  bed = read.csv(files[i], header = FALSE, sep = "\t")[, c(1, 2, 3, COLUMN)]
  colnames(bed) = c("chr", "start", "end", 'values')
  # bed$values = rep(1, nrow(bed))
  bed$values = ifelse(bed$values > MAX_SCORE, yes = MAX_SCORE, no = bed$values)

  col = colors[i]

  circos.genomicTrack(bed, ylim=c(MIN_SCORE, MAX_SCORE), bg.border = "#BABDBF",
                      panel.fun = function(region, value, ...) {
                        circos.genomicLines(region, value, type = "h", lwd = 0.25, col = col, ...)
...
labels = c("Z-DNA peaks", "ZBP1 peaks", "Overlaps")
