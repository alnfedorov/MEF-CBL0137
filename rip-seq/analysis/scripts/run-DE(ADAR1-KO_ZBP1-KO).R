...
# Run DEGReport
design = as.data.frame(colData(deseq))
ma = rlog(deseq)

# Select only genes showing changes in the expression OR being DEGs in the GSE118926
GSE118926 = args[3]
GSE118926 = read.csv(GSE118926, row.names = 1)
GSE118926 = GSE118926[GSE118926$padj < 0.05, ]
degs = c(degs, row.names(GSE118926))

degs = unique(degs)
ma = assay(ma[row.names(ma) %in% degs])
ma = ma[apply(ma, 1, var) != 0, ]

# Detect DEG clusters
res <- degPatterns(ma, design, time = "IFNb", minc=1)
filename = file.path(saveto, "IFNb: clusters.csv")
write.csv(res$normalized, filename)
